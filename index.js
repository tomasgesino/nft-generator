...
const startCreating = async () => {
  ...
  while (editionCount <= editionSize) {
    ...
    let newDna = createDna(layers, "original");
    while (!isDnaUnique(dnaListByRarity["original"], newDna)) {
      // recalculate dna as this has been used before.
      console.log('found duplicate DNA ' + newDna.join('-') + ', recalculate...');
      newDna = createDna(layers, "original");
    }
    console.log('- dna: ' + newDna.join('-'));

    // propagate information about required layer contained within config into a mapping object
    // = prepare for drawing
    let results = constructLayerToDna(newDna, layers, "original");
    let loadedElements = [];
    ...
    dnaListByRarity["original"].push(newDna);
    editionCount++;
  }
